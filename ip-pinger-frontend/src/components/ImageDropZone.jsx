...
//     try {
//         // Get the IPs to ping from the devicesList
//         const ipAddresses = devicesList.map(device => device.ipAddress).join(',');
//         if (!ipAddresses) return;

//         const response = await axios.get('http://localhost:8080/ping', {
//             params: { ips: ipAddresses },
//         });

//         const results = response.data;

//         // Update the devices list with the results from the ping
...
// // Interval to ping the IPs every 30 seconds
// useEffect(() => {
//     const intervalId = setInterval(() => {
//         if (devicesList.length > 0) {
//             pingIps(); // Call pingIps only if there are IPs to ping
//         }
//     }, 30000); // Set the interval to 30 seconds

//     return () => clearInterval(intervalId); // Cleanup on unmount
// }, [devicesList]); // Keep the interval effect independent of state changes

// // Initial ping when the component mounts
// useEffect(() => {
//     if (devicesList.length > 0) {
//         pingIps(); // Ping IPs once on mount if there are IPs
//     }
// }, []); // Runs after devicesList is set after loading from localStorage
...
export default function ImageDropZone() {
  ...
  color="error" 
  onClick={() => {
    localStorage.clear('pins'); // Clear all localStorage
    setImage(null);       // Reset image state
    setPins([]);          // Reset pins state
    setDevicesList([]);   // Reset devices state
  }}
>
  Clear Storage
</Button>
